...
export default function GardenMapCanvas({ gardenMap, zones = [], plantations = [], onZoneClick, onPlantationClick }) {
  ...
  const [ox, oy] = toSvgPoint([0, 0], resolvedHeight);
  const [maxX, maxY] = toSvgPoint([resolvedWidth, resolvedHeight], resolvedHeight);
  ...
  return (
    <svg viewBox={`0 0 ${resolvedWidth} ${resolvedHeight}`} preserveAspectRatio="xMidYMid meet" className="h-full w-full min-h-[300px]" style={{ border: "1px solid #ddd", borderRadius: 8 }}>
      <polygon points={polygonToSvgPoints(geometry, resolvedHeight)} fill="rgba(34,197,94,0.08)" stroke="rgb(22,163,74)" strokeWidth={Math.max(resolvedWidth, resolvedHeight) * 0.01} />
      {debug && (
        <g>
          <polygon points={polygonToSvgPoints(geometry, resolvedHeight)} fill="none" stroke="red" strokeWidth={Math.max(resolvedWidth, resolvedHeight) * 0.012} />
      ...
      {zones.map((zone, idx) => {
        ...
        return (
          <g key={zone.id} onClick={() => onZoneClick?.(zone)} style={{ cursor: "pointer" }}>
            <polygon points={polygonToSvgPoints(zoneGeometry, resolvedHeight)} fill={debug ? "rgba(0,0,255,0.1)" : `hsl(${(idx * 77) % 360} 70% 75% / 0.5)`} stroke={debug ? "blue" : "#355"} strokeWidth="0.4" />
